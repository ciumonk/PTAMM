...
void ModelsGame::Draw3D( const GLWindow2 &glWindow, Map &map, SE3<> se3CfromW)
{
  ...
  // Find the current model and draw the target
     ...
     Model3DS *p = mData.CurrentModel();
        ...
        {
		//p->MoveRight();
                if(p->count_move==50) {p->incr=-1;}
                else if(p->count_move==-1) {p->incr=1;}
                p->count_move+=p->incr; 
                if(p->incr==1){p->MoveLeft();p->MoveLeft();}else{p->MoveRight();p->MoveRight();}
 ...
 // Draw the bullets
  ...
  double dScale = 0.01;//GV3::get<double>("ShooterGame.BulletSize", "0.001", SILENT);
...
void ModelsGame::HandleKeyPress( std::string sKey )
{
  if( sKey == "Enter" )
  {
	...
	Model3DS *p = mData.CurrentModel();
        ...
        {
		//p->MoveRight();
	//	if(p->count_move==5) {p->incr=-1;}
	//	else if(p->count_move==-1) {p->incr=1;}
	//	p->count_move+=p->incr; 
	//	p->incr==1?p->MoveLeft():p->MoveRight();
        }
  }
}
